      ...
      <Tooltip title={getToolTip()} leaveTouchDelay={7000} enterTouchDelay={0} arrow placement="top" followCursor>
        <Paper
          ...
          sx={{
            ...
            p: p || (submitButtonVariant === 'orange-circle' ? (height === '56px' ? '6px 8px 6px 20px' : '4px 6px 4px 16px') : '5px 12px'),
            paddingRight: '6px',
            display: 'flex',
            alignItems: 'center',
        ...
        >
          {showSearchIcon && (
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              <img alt="search icon" src={SEARCH_ICON_SRC} width={height === '56px' ? '18px' : '14px'} style={{ opacity: '0.5' }} />
            </Box>
          )}
          ...
          {isLoading ? (
            ...
            >
              <MuiSearchIcon
                sx={{
                  fontSize: height === '56px' ? '22px' : '18px',
                  color: '#ffffff',
                }}
              />
            </IconButton>
