...
import type { FieldValues, FormState, Path, PathValue, UseControllerProps } from 'react-hook-form'
...
const Component = <T extends FieldValues>({
  ...
  return (
    <Controller
      {...restC}
      render={({ field: { onChange: onChangeI, name, value = restC?.defaultValue ?? items?.[0].value ?? '', ref } }) => (
        <FormControl fullWidth error={!!error} {...formControlProps}>
          {label && (
            <InputLabel size={selectProps.size === 'small' ? 'small' : 'normal'} id={labelId} {...inputLabelProps}>
              {label}
            </InputLabel>
          )}
          <Select
