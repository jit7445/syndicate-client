...
export default function RegisterFields({ handleClose, onSwitchToSignIn }: RegisterFieldsProps) {
  ...
  return (
    <Grid container spacing={2} mt="1px">
      <HookTextField
        ...
        textFieldProps={{ ...commonInputStyles, label: 'Full name', required: true }}
        gridProps={{ xs: 12 }}
      />
      <HookTextField
      ...
      <Grid item xs={12}>
        <button type="button" onClick={onSwitchToSignIn} className="text-sm text-accent-2 underline">
          Already have an account? Sign in
        </button>
      </Grid>

      <FormCancelSubmitBtns handleClose={handleClose} submitLabel="Register" />
    </Grid>
  )
}
