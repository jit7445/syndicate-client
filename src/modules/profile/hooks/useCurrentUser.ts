...
// GET /api/profile -> { userId, userName, email, mobile, ... }
// Currently sourced only from the JWT claims cached in storage at sign-in
// (see authUtils.processToken), so any field not in the token (e.g. mobile)
// is unavailable here until a dedicated profile endpoint exists.
