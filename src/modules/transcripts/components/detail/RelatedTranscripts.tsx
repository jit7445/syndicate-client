...
export default function RelatedTranscripts({ domain, excludeId }: RelatedTranscriptsProps) {
  ...
  const relatedTranscripts = useMemo(
    () => transcripts.filter((t) => t.domain === domain && t.id !== excludeId).slice(0, RELATED_COUNT),
  ...
  return (
    <div className="mt-10">
      <h2 className="text-lg font-bold text-text-primary">Similar transcripts</h2>
